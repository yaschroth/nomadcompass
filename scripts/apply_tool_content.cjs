...
const TOOLS = {
  'route.html': {
    ...
    faq: [
      ...
      ['Does it include flight prices?', 'Yes, once you add them. Each leg names the two airports you would actually fly between and links straight to a search for that route on your arrival date, and whatever fare you type in joins the trip total and the share link. We do not guess the number for you: a ticket price turns on the date, the airline and when you happen to look, so an estimate would be worse than an empty field. Distance, flight time and CO2 are computed either way.'],
