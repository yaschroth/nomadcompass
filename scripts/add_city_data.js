...
function updateCitiesData() {
  ...
  for (const [cityId, data] of Object.entries(CITY_DATA)) {
    // Pattern to find the city and add data after the scores closing brace
    const pattern = new RegExp(
      `(id: "${cityId}"[\\s\\S]*?scores: \\{[^}]+\\})`,
      'g'
    );

    const newContent = content.replace(pattern, (match) => {
      // Check if already has costPerMonth
      if (match.includes('costPerMonth')) {
        return match;
      }
      updatedCount++;
      return match + `,
  ...
  console.log(`Updated ${updatedCount} cities with cost and coordinate data`);
}
